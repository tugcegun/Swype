...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password.length < 6) {
      return setError(t('register.errorLength'))
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-swype-cream dark:bg-swype-dark px-4 transition-colors relative">
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <h1 className="text-lg font-bold text-swype-dark dark:text-swype-cream">{t('register.title')}</h1>
          <p className="text-xs text-swype-silver mt-1">{t('register.subtitle')}</p>
        </div>
        ...
        <div className="bg-white dark:bg-swype-mid/50 rounded-sm shadow-sm border border-swype-silver/30 dark:border-swype-mid p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <Input label={t('register.name')} type="text" value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder={t('register.namePlaceholder')} required />
            <Input label={t('register.email')} type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="ornek@email.com" required />
            <Input label={t('register.password')} type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t('register.passwordPlaceholder')} required />
            <Input label={t('register.confirmPassword')} type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder={t('register.confirmPlaceholder')} required />
        ...
        <p className="text-center text-xs text-swype-silver mt-5">
          ...
          <Link to="/login" className="text-swype-dark dark:text-swype-cream font-semibold hover:underline">
            {t('register.login')}
          </Link>
        </p>
      </div>
